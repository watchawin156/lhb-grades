...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  if (request.method === "POST") {
    try {
      ...
      subjects.forEach((s) => {
        if (!s.id || !s.code) return;
        gradeQueries.push(db.prepare("INSERT OR REPLACE INTO subjects (id, code, name, maxScore, semester, type, credit, class_level, year) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)").bind(s.id, s.code, s.name, s.maxScore || 100, s.semester || 1, s.type || "\u0E1E\u0E37\u0E49\u0E19\u0E10\u0E32\u0E19", s.credit || 1, s.class_level || null, s.year ?? null));
      });
      if (scores.length > 0) {
        gradeQueries.push(db.prepare("DELETE FROM scores"));
        scores.forEach((s) => {
          if (!s.studentId || !s.subjectId) return;
          if (s.score1 !== void 0 && s.score1 !== null) {
            gradeQueries.push(db.prepare("INSERT INTO scores (student_internal_id, subject_internal_id, score, academic_year, semester) VALUES (?, ?, ?, ?, ?)").bind(s.studentId, s.subjectId, s.score1, Number(s.year) || 2568, 1));
          }
          if (s.score2 !== void 0 && s.score2 !== null) {
            gradeQueries.push(db.prepare("INSERT INTO scores (student_internal_id, subject_internal_id, score, academic_year, semester) VALUES (?, ?, ?, ?, ?)").bind(s.studentId, s.subjectId, s.score2, Number(s.year) || 2568, 2));
          }
        });
      }
      const promises = [];
...
// ../.wrangler/tmp/pages-GgDeM5/functionsRoutes-0.38249624022132667.mjs
var routes = [
  {
    routePath: "/api/data",
    mountPath: "/api",
