...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  if (request.method === "GET") {
    try {
      ...
      const d1Scores = results[2].results;
      const scoreMap = {};
      d1Scores.forEach((s) => {
  ...
  if (request.method === "POST") {
    try {
      ...
      students.forEach((s) => {
        queries.push(db.prepare("INSERT INTO students (id, studentId, name, class, room, number, year) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(s.id, s.code, s.name, s.class, s.room || null, s.number || null, s.year));
      });
      queries.push(db.prepare("DELETE FROM subjects"));
      subjects.forEach((s) => {
        queries.push(db.prepare("INSERT INTO subjects (id, code, name, maxScore, semester, type, credit) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(s.id, s.code, s.name, s.maxScore, s.semester, s.type || "\u0E1E\u0E37\u0E49\u0E19\u0E10\u0E32\u0E19", s.credit || 1));
      });
      queries.push(db.prepare("DELETE FROM scores"));
      scores.forEach((s) => {
        if (s.score1 !== void 0) {
          queries.push(db.prepare("INSERT INTO scores (student_internal_id, subject_internal_id, score, academic_year, semester) VALUES (?, ?, ?, ?, ?)").bind(s.studentId, s.subjectId, s.score1, Number(s.year), 1));
        }
        if (s.score2 !== void 0) {
          queries.push(db.prepare("INSERT INTO scores (student_internal_id, subject_internal_id, score, academic_year, semester) VALUES (?, ?, ?, ?, ?)").bind(s.studentId, s.subjectId, s.score2, Number(s.year), 2));
        }
      });
...
// ../.wrangler/tmp/pages-NZwvKK/functionsRoutes-0.16242247863588888.mjs
var routes = [
  {
    routePath: "/api/data",
    mountPath: "/api",
