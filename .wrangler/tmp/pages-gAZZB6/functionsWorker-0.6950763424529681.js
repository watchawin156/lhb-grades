...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  if (request.method === "GET") {
    try {
      const [stdRes, subRes, scoRes] = await Promise.all([
        sdb.prepare("SELECT * FROM students").all(),
  ...
  if (request.method === "POST") {
    try {
      ...
      subjects.forEach((s) => {
        gradeQueries.push(db.prepare("INSERT INTO subjects (id, code, name, maxScore, semester, type, credit) VALUES (?, ?, ?, ?, ?, ?, ?)").bind(s.id, s.code, s.name, s.maxScore, s.semester, s.type || "\u0E1E\u0E37\u0E49\u0E19\u0E10\u0E32\u0E19", s.credit || 1));
      });
...
// ../.wrangler/tmp/pages-gAZZB6/functionsRoutes-0.9059466646070501.mjs
var routes = [
  {
    routePath: "/api/data",
    mountPath: "/api",
